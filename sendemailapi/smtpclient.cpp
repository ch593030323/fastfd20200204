...
QString SmtpClient::trans_GBKC2UTF8(const QByteArray &in)
{
    QTextCodec *c = QTextCodec::codecForName("GBK");
    return c->toUnicode(in);
}
QByteArray SmtpClient::trans_UTF82GBK(const QString &in)
{
    QTextCodec *c = QTextCodec::codecForName("GBK");
...
void SmtpClient::waitForResponse() throw (ResponseTimeoutException)
{
    do {
        ...
        {
            qDebug() << __FILE__ << __LINE__ << socket->errorString();
        ...
        while (socket->canReadLine()) {
            ...
            responseText = trans_GBKC2UTF8(line);
            qDebug() << __FILE__ << __LINE__ << "responseText" << responseText;
...
void SmtpClient::sendMessage(const QString &text)
{
    ...
    qDebug() << __FILE__ << __LINE__ << "sendMessage" << text;
}
