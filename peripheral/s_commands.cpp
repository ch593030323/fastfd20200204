...
AddShapeCommand::AddShapeCommand(S_Document *doc, const Shape &shape, QUndoCommand *parent)
    : QUndoCommand(parent)
{
    m_doc = doc;
    m_shape = shape;
...
RemoveShapeCommand::RemoveShapeCommand(S_Document *doc, const QString &shapeName,
                                        QUndoCommand *parent)
    : QUndoCommand(parent)
{
...
SetShapeColorCommand::SetShapeColorCommand(S_Document *doc, const QString &shapeName,
...
{
    if (command->id() != setShapeColorCommandId)  return false;

    const SetShapeColorCommand *other = static_cast<const SetShapeColorCommand*>(command);
    if (m_shapeName != other->m_shapeName) return false;
...
SetShapeRectCommand::SetShapeRectCommand(S_Document *doc, const QString &shapeName,
...
{
    if (command->id() != setShapeRectCommandId)   return false;
