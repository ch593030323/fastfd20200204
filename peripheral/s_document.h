...
{
public:
    enum Type { Rectangle, Circle, Triangle, TwoBarCode, OneBarCode, OTHER};

    explicit Shape(Type type = Rectangle, const QColor &color = Qt::red
            , const QRect &rect = QRect(), const QString &text = QString()
            ,const QString &fsize = QString(), bool ismoving = true
            ,bool istext=false
            ,int rotate = 0);
...
private:
    ...
    QColor m_color;
    QString m_name;
    friend class S_Document;
};

class S_Document : public QWidget
{
    Q_OBJECT

public:
    S_Document(QWidget *parent = 0);
    ...
    void setShapefontsize(const QString &shapeName, const QString &fs);
    void setShapeistext(const QString &shapeName, bool value);
    void setShapeRotate(const QString &shapeName, int rotate);
...
signals:
    ...
    void currentShapeChanged(const QRect &rect);
    void ShapeClicked(const QRect &rect);
